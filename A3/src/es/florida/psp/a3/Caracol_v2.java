...
public class Caracol_v2 implements Runnable {
	...
	public static void main(String[] args) {
		...
		Caracol_v2 objetoCaracol_v2;
		Thread hiloCaracol_v2;
		for (int i = 0; i < nombres.length; i++) {
			objetoCaracol_v2 = new Caracol_v2(nombres[i], velocidades[i]);
			hiloCaracol_v2 = new Thread(objetoCaracol_v2);
	...
	public void run() {
		...
		while (avance < distancia) {
			...
			System.out.println("El caracol " + nombre + " ha avanzado " + avance + " metros, " + porcentaje + "%");
			// try {
			// 	Thread.sleep(1000);
